...
| A balcony with an empty `rowLabels` array rendered placeholders because inventory rows used `BALC-A...` while the UI generated `A...`. | Generate `BALC-`-prefixed fallback labels for balcony inventory while preserving explicitly configured labels. | Live event API inspection; balcony fallback integration test |
