...
- Removed the unused frontend TypeScript error dumps and dummy receipt after confirming they had no runtime or test references. The empty `tmp/concurrent_booking_test.ts`, default framework icons and historical IDE HTTP responses remain artifact candidates and should be handled in their owning backend/repository cleanup.
